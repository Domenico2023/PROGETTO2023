...
{
    ifstream file;
    file.open(inputFilePath);  //apro il file

    if(file.fail())
        return false;  //check che tutto sia a posto
                ...
                const double& V)
{
    ...
    out << "r = [" << " ";
    for (unsigned int i =0; i<n; i++){
        if (ceil(r[i]*10)==r[i]*10){
            out << setprecision(1) << r[i]<< " ";
        }
        else{
            out << setprecision (2) << r[i] << " ";
        }
    }
    out << "]" << endl;
    out.precision(4);
