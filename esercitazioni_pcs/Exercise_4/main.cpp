...
bool ImportCell0Ds(PolygonalMesh& mesh)
{
  ...
  {
    //line.replace(line.begin(),line.end(),';',' ');   //purtroppo non va :(
...
bool ImportCell2Ds(PolygonalMesh& mesh)
{
  ...
  for(const string& line : listLines)
  {
    ...
    for(unsigned int i = 0; i < n_ver; i++) {getline(converter, row,';'); vertices.push_back(stoi(row));}
    getline(converter, row, ';'); n_edg = stoi(row);
    for(unsigned int i = 0; i < n_edg; i++) {getline(converter, row, ';'); edges.push_back(stoi(row));}
